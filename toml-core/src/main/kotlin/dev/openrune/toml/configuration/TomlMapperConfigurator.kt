...
import dev.openrune.toml.transcoding.TomlDecoderRegistry.modifiers
...
class TomlMapperConfigurator internal constructor(
    ...
    inline fun <reified T : Any> addDecoder(noinline modify: (Map<String, TomlValue>, T) -> Unit) {
        val wrapper: (Map<String, TomlValue>, Any) -> Unit = { map, instance ->
            @Suppress("UNCHECKED_CAST")
            modify(map, instance as T)
        }
        modifiers.getOrPut(T::class) { mutableListOf() }.add(wrapper)
    }

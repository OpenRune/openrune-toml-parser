...
sealed class TomlException : RuntimeException() {
    ...
    sealed class DecodingError(
        ...
        class IllegalMapKeyType(tomlMap: TomlValue.Map, targetMapType: KType) : DecodingError(
            "Tried to decode object into map with illegal key type " +
                ...
                "(use @param:TomlField(serializer = ...) for custom key rules, e.g. integer table keys).",
            tomlMap,
            targetMapType
        )
    ...
    /**
     * Thrown when a [dev.openrune.toml.serialization.TomlField] references a serializer class that cannot be instantiated.
     */
    data class InvalidTomlFieldSerializer(
        val serializerClass: kotlin.reflect.KClass<*>,
        override val cause: Throwable? = null,
    ) : TomlException() {
        override val message: String
            get() = "Cannot instantiate TomlFieldSerializer '${serializerClass.qualifiedName ?: serializerClass.simpleName}' " +
                "(declare it as an `object`, or provide a public no-arg constructor)."
    }
